...
// Modified for Dual Mode (HID/MIDI) Support
// ===================================================================================

#include "ch554.h"
#include "usb_handler.h"
#include "usb_descr_midi.h"      // MIDI描述符
#include "config.h"               // 模式定义
          ...
          switch(USB_setupBuf->wValueH) {
            ...
            case USB_DESCR_TYP_DEVICE:            // Device Descriptor
              ...
              if(device_mode == MODE_MIDI) {
                pDescr = (uint8_t*)&DevDescr_MIDI;
                len = sizeof(DevDescr_MIDI);
              } else {
            ...
            case USB_DESCR_TYP_CONFIG:            // Configuration Descriptor
              // 根据模式选择对应的配置描述符
              if(device_mode == MODE_MIDI) {
                pDescr = (uint8_t*)&CfgDescr_MIDI;
                len = sizeof(CfgDescr_MIDI);
              } else {
